...
```{r ET-stats-part-1, eval= TRUE, echo = FALSE,include = TRUE, message=FALSE, warning=FALSE, results = 'asis'}
    ...
    ## generate summary statistics for each year of ET results
     ...
     ### STOPPED HERE 20-MAR-2021 ###

    # convert from list to dataframe

    Ref_ET_data_df <- ldply(Ref_ET_data_list_df)

    # seq_along the Years and get the summary statistics by Year; save in a list
    

     # create empty list
      
      ET_rc_data_smry_list <- list(NULL)
      
      # get the unique Years from the dataframe of results
      
      unique_years <- unique(Ref_ET_data_df$Year)
      
      # seq_along the unique years
      
      for (q in seq_along(unique_years)) {  
      
        # filter for the Year
          
        data_one_year <- Ref_ET_data_df %>% filter(Year==unique_years[q])
      
        # apply hydroTSM:smry; assign results to a list
        
        ET_rc_data_smry_list[q] <- hydroTSM::smry(data_one_year$ET_rc)
      ...
      names(ET_rc_data_smry_list) <- as.character(unique_years)
      
      
      # use 'bind_cols' to combine the list of dataframes to one dataframe

      ET_rc_data_smry_bind_cols <- bind_cols(ET_rc_data_smry_list)

      # convert from tibble to dataframe
      
      ET_rc_data_smry_bind_cols <- as.data.frame(ET_rc_data_smry_bind_cols)
    ...
    # function to select only the Year and ET_rc

    select_two_cols <- function(xx){
      
      xx %>% select(c(Year,ET_rc))
      
      
    } # end of 'select_two_cols' function
      
     
      # use 'map' to apply 'select_two_cols' to the list of dataframes

      # Ref_ET_data_list_df <- Ref_ET_data_list_df %>% map(select_two_cols)

      # view(Ref_ET_data_list_df)
      

      # use 'map' to apply 'smry' to the list of dataframes

      Ref_ET_stats_list <- Ref_ET_data_list_df %>% map(hydroTSM::smry)

      # use 'bind_cols' to combine the list of dataframes to one dataframe

      Ref_ET_stats_bind_cols <- bind_cols(Ref_ET_stats_list)
      
      # cbind in the rownames so they appear when you knitr to Word
      
      Ref_ET_stats_bind_cols <- cbind(rownames(Ref_ET_stats_bind_cols),Ref_ET_stats_bind_cols)

      # add column names
      # colnames(Ref_ET_stats_bind_cols) <- c("Hourly","Daily","Weekly","Monthly","Yearly")
      
      # colnames(Ref_ET_stats_bind_cols) <- c("Statistic","YYC-2016","YYC-2017","Weekly","Monthly","Yearly")
      
      # colnames(Ref_ET_stats_bind_cols) <- c("Statistic","YYC-2016","YYC-2017")

      # add Site.Name to the dataframe - this may be optional - under review

      # Site.Name <- rep(site_prefix,nrow(Ref_ET_stats_bind_cols))
      # 
      # Ref_ET_stats_bind_cols<- data.frame(Site.Name,Ref_ET_stats_bind_cols)
      ...
      # write the results to .CSV file
      write.table(Ref_ET_stats_bind_cols,CSV_outfile,sep=",",col.names=NA,row.names=TRUE)
