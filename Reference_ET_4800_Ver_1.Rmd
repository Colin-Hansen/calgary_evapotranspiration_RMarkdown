...
```{r ET-stats-part-1, eval= FALSE, echo = FALSE,include = TRUE, message=FALSE, warning=FALSE, results = 'asis'}


    # read the ET results .csv files into a list
  
    # get the names of the .csv results files
    results_filenames<-dir('data_out/Ref_RET_Results_1',full.names = TRUE)
    
   
    # enter loop to create a list of the results filenames; ** may not need this **
    
    # create empty list
    
    results_filenames_list <- list(NULL)
    
    for (q in seq_along(results_filenames)) {  
    
    # build the list of raw data filenames 
      
    results_filenames_list[q] <- list(results_filenames[q])
    
      } # end of 'seq_along(results_filenames)' 
    
    
    
    
    
    # enter loop to create a list of the ET results - 'smry' stats
    
    # create empty list
    
    smry_stats_list <- list(NULL)
    
    
    for (w in seq_along(results_filenames)) {
  
        # read the ET results .csv file
        ET_results<-read.csv(results_filenames[w],header = TRUE,sep=",",stringsAsFactors = FALSE)
      
        # get the summary statistics using 'smry'
        smry_stats_list[w] <- hydroTSM::smry(ET_results$ET_rc)
