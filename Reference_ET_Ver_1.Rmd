...
Historical rainfall data from the City of Calgary precipitation monitoring network is 
...
This project uses R software to read the historical rainfall data for each rain gauge site for the 
period 1988-2019. In the first step, the data for each site is compiled into a single .csv file. 
In the subsequent steps, various summary statistics and charts are created for each site.
In the final step, the results are exported to a PDF file (one file for each rain gauge site).

Discrete rainfall events are identified by specifying the Inter-Event Time (IET), which is 
the minimum elasped time between rainfall events (i.e. the dry period between events). IET
values of 60 minutes and 360 minutes were used in this project, based on a review of the 
literature. 

The analysis for each site includes a comparison between the yearly rainfall at the City rain gauge versus
the rainfall from the Environment Canada gauge at Calgary International Airport. 
...
**Rain Gauge Equipment:** Tipping Bucket (41); Weighing Type (4)

**Number of Locations:** 45 sites
...
```{r compile-raw-data, eval = TRUE, include=FALSE}
...
rm(list=ls())
  ...
  single_site_name <- site_names[01]
  
  
  # get the names of the raw data subfolders
  subfolder_names<-dir('data_in',full.names = TRUE)
  
  # subfolder_names<-subfolder_names[c(1:25,31:32)] # TESTING ONLY
  
  subfolder_names<-subfolder_names[5:9] # TESTING ONLY
  ...
  site_name_detect <- str_detect(vector_all_filenames_unlist,single_site_name)
...
## Note: dashed line is the Calgary Intl. 'A' Normal Rainfall for May to Sept. (1981-2010)
...
```{r charts-part-2, eval= TRUE,echo = FALSE,include = TRUE, message=FALSE, warning=FALSE, results = 'asis',fig.width=10,fig.asp=0.618}
 ...
 # plot May - Sept 'annual' rainfall data - plot in descending order
  ...
  p2<-ggplot(data=rain_yearly, aes(x = reorder(Year, -Rainfall.mm),y=Rainfall.mm,fill=Year)) +
    ...
    theme(axis.title.x = element_text(size = 12),axis.text.x = element_text(angle=45,hjust=1)) +
    theme(axis.title.y = element_text(size = 12)) +
    ## add YYC May - Sept normal rainfall
    geom_hline(aes(yintercept=304.2),color="red",linetype="dashed") +
    ylab("Rainfall (mm)") +
...
```{r charts-part-5,eval= TRUE,echo = FALSE,message=FALSE, warning=FALSE}
  ...
  ## Note: DO NOT MESS MUCH WITH THE FOLLOWING CODE FOR the p5 plot. It took a while to get it looking right. 
      ...
      # create chart title (dynamic)
      chart_title<-paste0("Rain Gauge"," ",site_prefix," ", "Yearly May - September Rainfall vs. Calgary Intl. Airport")
...
## Rainfall Event Statistics - IETD 60 Minutes
...
# the read function was removed because the 5-minute rainfall data was already read in a previous chunk
    ...
    for (i in seq_along(MIT_value)) {
      ...
      ddi_list_df <- ddi_list %>% map(as.data.frame)
      
      
      # use 'map' instead of seq_along 'ddi_list_df' to apply 'smry' to the list of dataframes
      
      stats_df_list <- ddi_list_df %>% map(hydroTSM::smry)
    ...
    ## ADDED 5-MARCH-2021
      ...
      # save the 'ddf_list' results to a .csv file; ONLY IF MIT = 60
       
       if(MIT_value[i] == 60) {
         
          # use 'bind_cols' to combine the list of tibbles to one tibble
          
          ddi_list_combined <- bind_cols(ddi_list)
...
## Boxplot of Rainfall Event Depth, Duration (IETD 60 Minutes)


```{r event-boxplot-part-1, eval=TRUE, echo = FALSE,message=FALSE, warning=FALSE}

  # create boxplot of the rainfall event depth, duration, and intensity; MIT 60 only

  # source: 'ddi_list_combined' in previous chunk

  
   # use 'gather' to re-format the results to long format (suitable for ggplot)
 
  gathered_ddi_results <- ddi_list_combined %>% gather(key=Parameter,value=MIT.result,depth,duration,intensity)


    # boxplot of the three results
      ...
      chart_title<-paste0(site_prefix," ", "Rain Event Results - IETD 60")
    
      # create chart PNG outifle name (dynamic)
      # outfile_name<-paste0(site_prefix,"_", "MIT_", MIT_value[i],"_Boxplot.png")
     
      # this bolxplot requires the 'scales'package ('log10')
     
      p8<-ggplot(data=gathered_ddi_results, aes(x=Parameter,y=MIT.result,fill=Parameter)) +
        geom_boxplot() +
...
## Rainfall Event Statistics (IETD 360 Minutes)
...
## Rainfall Maximum Gap Times - Inter-Event Time (IETD) 60 Minutes
...
## Rainfall Gap Time Statistics - IETD 60 Minutes

## Note: output maximum of 10 years of data - for brevity
...
```{r gap-times-table-1, eval=TRUE, echo = FALSE,message=FALSE, warning=FALSE}
      ...
      names(table_data_4) <- c("Statistic",years_char)
        ...
        # use the 'flextable' package to create MASTER table (e.g. MIT 60)
      ...
      ############## If block #1 ########################
        ...
        # output table if at least 10 columns
      
        if (last_col >= 11) {
      
        t1 <- table_data_4[,1:11]
      ...
      # output a smaller table if less than 10 columns
      
        if (last_col < 11) {
      
        t1 <- table_data_4[,1:last_col]
